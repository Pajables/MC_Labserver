...
def add_reaction():
    if request.method == "GET":
        ...
        all_reactions = utils.get_avail_reactions(db)
        all_reactions = [item for item in set(all_reactions)]
        sql = f"CREATE TABLE IF NOT EXISTS {table_name} (LAST_UPDATE_DATE TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, REACTION_ID INT, USER_ID INT,"
